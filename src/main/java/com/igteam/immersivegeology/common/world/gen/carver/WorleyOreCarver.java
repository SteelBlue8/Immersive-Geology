...
public class WorleyOreCarver {
	...
	public static INoise3D[] finalNoiseData = new INoise3D[EnumMaterials.values().length];
	...
	public void createOreType(Random seedGenerator, EnumMaterials ore) {
			...
			finalNoiseData[ore.ordinal()] = (oreNoise.sub(oreNoiseSub));
	}

	public void carve(IChunk chunkIn, int chunkX, int chunkZ) {

		Biome foundBiome = chunkIn.getBiome(chunkIn.getPos().asBlockPos());
		BiomeLayerData biomeData = null;
		for(BiomeLayerData b : WorldLayerData.instance.getData()){
			if(b.getLbiome().getRegistryName().equals(foundBiome.getRegistryName())){
				biomeData = b;
			}
		}

		if(biomeData == null){
			IGLogger.info("Initial Biome check failed");
		}
		...
		// run this through a full loop, for each ore, returns out if ore is not set in the biomes layer data!
			...
			for (int currentLayer = totalLayerCount; currentLayer > 0; currentLayer--) {
				...
				ArrayList<LayerOre> oreArrayData = new ArrayList<>();
				if(biomeData.getLayerOre(currentLayer) != null) {
					oreArrayData.addAll(biomeData.getLayerOre(currentLayer));
					if (oreArrayData.isEmpty()) {
						IGLogger.error("No Ore Data is found for Biome: " + biomeData.getLbiome().getRegistryName().getPath());
						return;
					}

					for (LayerOre oreData : oreArrayData) {
						if (oreData == null) {
							IGLogger.error("Ore Data is missing for biome:" + biomeData.getLbiome().getRegistryName().getNamespace());
							return;
						}
						...
						INoise3D oreNoise = finalNoiseData[oreData.getOre().ordinal()];
											...
											for (int x0 = x * 4; x0 < (x + 1) * 4; x0++) {
												for (int z0 = z * 4; z0 < (z + 1) * 4; z0++) {
													...
													if ((yPos <= topOfLayer) && (yPos >= bottomOfLayer)) {
														Material baseMaterial = biomeData.getLayerBlock(currentLayer).getMaterial();
														Block foundBlock = chunkIn.getBlockState(pos).getBlock();
														if(foundBlock instanceof IGMaterialBlock){
															IGMaterialBlock matBlock = (IGMaterialBlock) foundBlock;
															baseMaterial = matBlock.getMaterial();
														}
														...
														if (richnessNoise > NOISE_THRESHOLD) {
															...
															if ((originalState.getBlock() instanceof IGMaterialBlock) && !(originalState.getBlock() instanceof IIGOreBlock) && !originalState.isAir() &! originalState.getMaterial().equals(net.minecraft.block.material.Material.WATER)) {
